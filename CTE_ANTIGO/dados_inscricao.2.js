...
function validaCampos(){
	...
	}else if(($.trim($("input[name='CNPJ']").val()) == '') && ($('input[type=radio][name=emissao_em_nome]:eq(0)').is(':checked'))){
		alert('O campo "CNPJ" é de preenchimento obrigatório, quando selecionado opção "Empresa".');
		$("input[name='CNPJ']").focus();
		return false;		
...
function calcularValorInscricao(){
	...
	if($("input[name='emissao_tipo_comprovante'][value='NF']").is(':checked') && (document.forms[0].CNPJ.value != "" || document.forms[0].CPF.value != "" )){
			
		if(cidade == "saopaulo"){
			IRRIS 		= (parseFloat(valorTotalInscricao) > 667.00) ? parseFloat(ISS) + parseFloat(IR) : parseFloat(ISS);
			valorIRRIS 	= ((parseFloat(valorTotalInscricao) * parseFloat(IRRIS))/100);
			//alert('ISS = '+ISS+' / '+'IR = '+IR+' | IRRIS = '+IRRIS);
			//alert('valorIRRIS 7 = '+valorIRRIS);
		}else{
			// alert('Else 2');
			// alert('valorTotalInscricao = '+valorTotalInscricao);
			if(parseFloat(valorTotalInscricao) > 667.00)
				valorIRRIS = ( (parseFloat(valorTotalInscricao) * parseFloat(IR))/100 );
			else
				valorIRRIS = 0.0;
		}
	...
	//O desconto só é realizado se o imposto for superior a R$9,99
	valorFinal = (parseFloat(valorIRRIS) >= 10) ? valorTotalInscricao - valorIRRIS : valorTotalInscricao; //DARF	
