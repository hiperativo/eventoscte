<?php
	...
	// Seleciona todas as datas de inscriчуo do evento
	...
	$query_data_inscricao = "SELECT 
								  ...
								  WHERE cte_ev.idEvento = (SELECT idEvento FROM cte_evento WHERE dt_evento >= DATE(NOW()) AND bit_ativado = 1 ORDER BY dt_evento ASC LIMIT 1)
								  AND data_inscricao >= DATE(NOW())
								  AND cte_di.bit_ativado = 1
	...
	//Seleciona todos os tipos de inscriчѕes
	...
	$i= 0;
		...
		while($row_tipos_inscricoes = mysql_fetch_array($result_tipos_inscricoes)){
			...
			$query_valores_inscricao = "SELECT cte_evp.*,cte_edi.idDataInscricao AS idDataInscricao_data_inscricao FROM cte_evento_data_inscricao cte_edi
						...
						WHERE idEvento = (SELECT idEvento FROM cte_evento WHERE dt_evento >= DATE(NOW()) AND bit_ativado = 1 ORDER BY dt_evento ASC LIMIT 1)
						AND data_inscricao >= DATE(NOW())
						AND cte_edi.bit_ativado = 1
						ORDER BY data_inscricao ASC";
